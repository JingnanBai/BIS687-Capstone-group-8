...
In this study, we will categorize the response variable as binary
outcomes (0 for non-PD, 1 for PD) at each participant's visit, enabling
an in-depth exploration of potential risk factors through statistical
...
\hypertarget{specific-aim-2-building-practical-and-interpretable-models-for-predicting-the-risk-of-parkinsons-disease-for-each-specific-patient.}{%
\subsection{Specific Aim 2: Building practical and interpretable models
for predicting the risk of Parkinson's disease for each specific
patient.}\label{specific-aim-2-building-practical-and-interpretable-models-for-predicting-the-risk-of-parkinsons-disease-for-each-specific-patient.}}
...
will build predictive models for each specific patient to estimate the
risk of Parkinson's disease, while exploring more indicators that may
not be causative but could assist in early Parkinson's diagnosis.
...
Given the extreme class imbalance and the complex interactions between
among risk factors of Parkinson's disease, oversampling techniques may
introduce ambiguity to classification boundaries and exacerbate
potential confusion and overfitting, while training only with samples
after matching may also introduce overfitting, as the distribution of
training data is different from the practical scenario. To address the
possible challenges, we plan to employ matching methods with appropriate
cost-sensitive learning algorithms as alternative approaches to mitigate
these issues.
